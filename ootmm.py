...
class PathTable:
	class TableEntry:
		def __init__(self, path, dist, exit_number):
			self.path = path
			self.dist = dist
	...
	def generate_paths(self, flags, base):
		def empty_table(k, n):
			...
			for i in range(k):
				for j in range(n):
					table[i][j] = self.TableEntry(None, None, None)
			return table

		def copy_base_table():
			table = [[self.TableEntry(cell.path, cell.dist, cell.exit_number) for cell in row] for row in self.base_table]
			return table
	...
	def find_route(self, flags_index, start, end):
		...
		path = table[s][e].path
		dist = table[s][e].dist
		exit_number = table[s][e].exit_number
		...
		if dist == 1:
			...
			return Route(exit_number, [start, path, end])
